...
class CountriesOrder(unittest.TestCase):
    ...
    def test_countries_order(self):
        ...
        self.assertTrue(countries == sorted(countries), "Countries are not in the alphabetical order")
        ...
        for country in countries_with_zones:
            ...
            zones = [sub_row.find_element(By.CSS_SELECTOR, "td:nth-child(3)>input[type=hidden]").get_attribute("value")
                     for sub_row in sub_rows]
            self.assertTrue(zones == sorted(zones), "Zones are not in the alphabetical order")
        ...
        for geo_zone in geo_zones:
            ...
            zones = [
                sub_row.find_element(By.XPATH, ".//td[3]//option[@selected='selected']").get_attribute("textContent")
                for sub_row in zones_rows]
            self.assertTrue(zones == sorted(zones), "Zones are not in the alphabetical order")
            driver.find_element_by_link_text('Geo Zones').click()
