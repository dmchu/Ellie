...
class UserRegistration(unittest.TestCase):
    ...
    def test_add_new_account(self):
        ...
        driver.execute_script("arguments[0].selectedIndex = {}; "
                              "arguments[0].dispatchEvent(new Event('change'))".format(countries.index(country)),
                              select_country)
        # Select State
        ...
        self.assertTrue(success_create.text == "Your customer account has been created.", "account has not been created")
        ...
        self.assertTrue(success_login.text == "You are now logged in as {0} {1}.".format(user_details['First Name'],
                                                                                    user_details['Last Name']),
                   "User's full name doesn't much!")
